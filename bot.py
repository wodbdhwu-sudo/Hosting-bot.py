...
class Btn(types.InlineKeyboardButton):
    """InlineKeyboardButton — style colors DISABLED (always neutral)."""
    def __init__(self, *args, style: str = "", **kwargs):
        # Ignore style=... so buttons never render green/blue/red.
...
# │  BOT TOKEN  add karo.   ││
# └──────────────────────────────────────────────────────────────┘
BOT_TOKEN_HARDCODED = "8591537357:AAGHrOndhQIuykIOiR7z_jyghC2gvVtmXaA"   # ← ADD BOT TOKEN
TOKEN = (
...
try:
    OWNER_ID = int(os.environ.get("OWNER_ID", "6250965641"))
...
BRAND       = "SIR_LINUXX HOSTING"
BRAND_VER   = "v2.1"
BRAND_TAG   = f"{BRAND} {BRAND_VER}"
SUPPORT_USR = "@SIR_LINUXX"
UPDATE_CH   = "https://t.me/SIR_LINUXX"
...
def render_payment_methods_for(call: types.CallbackQuery, plan: str) -> None:
    ...
    cap = (
        ...
        f"{G['div']}\n"
        f"📩 {sc('Get code from admin')}: @SIR_LINUXX\n"
        f"{FOOTER}"
    )
    kb = types.InlineKeyboardMarkup()
    kb.add(Btn(f"{G['ticket']}  {sc('Enter Coupon Code')}", callback_data="menu_coupon", style="success"))
...
_HELP_PAGES = {
    "main": {
        "title": "📚 Help Centre",
        "text": (
            "Welcome to <b>SIR_LINUXX HOSTING</b>\n\n"
            "<b>Quick Start</b>\n"
...
def render_referral(call: types.CallbackQuery) -> None:
    ...
    try:
        ...
        link = f"https://t.me/SIR_LINUXX?start={uid}"
    cap = (
        f"<b>{G['users']} {sc('Referral')}</b>\n"
        f"{G['div_eq']}\n"
        f"{bullet('Your link', link)}\n"
